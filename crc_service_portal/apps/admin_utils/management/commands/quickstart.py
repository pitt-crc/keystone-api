...
class Command(BaseCommand):
    ...
    def run_gunicorn(self, host: str = '0.0.0.0', port: int = 8000):
        ...
        command = ['gunicorn', '--bind', f'{host}:{port}', 'crc_service_portal.main.wsgi:application', ]
    ...
    def run_uvicorn(self, host: str = '0.0.0.0', port: int = 8000):
        ...
        command = ['uvicorn', 'crc_service_portal.main.wsgi:application', '--host', host, '--port', str(port)]
        subprocess.run(command, check=True)
