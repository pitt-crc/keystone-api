...
class List(APITestCase):
    ...
    def test_regular_user(self) -> None:
        ...
        for username in ('user1', 'user2', 'common_user'):
            ...
            expected_records = ProposalReview.objects.affiliated_with_user(user).all()
            response = self.client.get(ENDPOINT)
            self.assertEqual(ProposalReviewSerializer(expected_records, many=True).data, response.data)
    ...
    def test_staff_user(self) -> None:
        ...
        expected_records = ProposalReview.objects.all()
        expected_data = ProposalReviewSerializer(expected_records, many=True).data
